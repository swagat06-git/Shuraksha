...
function dot(
  ...
  const pulseClass = severity ? `shuraksha-marker--${severity}` : "";

  return L.divIcon({
    className: "",
    html: `<div class="shuraksha-marker ${pulseClass}" style="--marker-color:${color};width:${size}px;height:${size}px;background:${color}">${glyph}</div>`,
    iconSize: [size, size],
...
function IncidentHeatmap({ reports }: { reports: Report[] }) {
  const map = useMap();
    ...
    >
      ...
      {reports.map((r) => (
        <Marker
          ...
          position={[r.location.lat, r.location.lng]}
          icon={dot(
            SEVERITY_VAR[r.severity],
            "!",
            r.severity === "critical" ? 30 : 24,
            r.severity,
          )}
          eventHandlers={{
        ...
        >
          <Popup>
            <strong className="capitalize">{r.type}</strong> · {r.severity} (
            {r.severityScore})
            <br />
            {r.address}
            <br />
